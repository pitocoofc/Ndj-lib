Delete the plain text content.
Delete o conteudo de texto puro


PT: Este arquivo é minha solução para o bug de copy... basicamente agora ja vem com o código fonte que você pode alterar
This is my solution for the copy bug... Now with a file that you only need to modify to configure your bot.


const { EasyBot } = require('easy-djs-bot');

const bot = new EasyBot({
    token: 'Bot_Token' // Sem o >
});


// Defina suas configurações globais aqui
bot.config = {
    ownerId: 'User_ID or 999 for null', // Pegue seu ID no Discord (botão direito no seu perfil)
